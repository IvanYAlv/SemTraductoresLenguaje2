...
class Variables:
    ...
    def __repr__(self):
        aux = ("Variable " + "\"" + str(self.context) + "\"" + str(self.cad))
        return aux
...
class retorno:
    ...
    def __repr__(self):
        aux = ("Valor/Variable: " + "\"" + str(self.context) + "\"" + str(self.cad) + " Tipo: " + str(self.tipo))
        return aux
...
class DefVar(Nodo):
    ...
    def __repr__(self):
        aux = ("Variable " + "\"" + str(self.lv) + "\"" + "\n \t \t \t" "Tipo: "+str(self.data.cad)+ " ID: "+str(self.tipo.cad))
        return aux
...
class LlamadaFunc(Nodo):
    ...
    def eliminallamada(self):
        ...
        if len(listaparametrosid)!=0:
            ...
            if bandera == 1:
                listaerrores.append('Error en la llamada a la funcion ' + self.data.cad + ' | Puede haber tipos de datos diferentes')
                globals()['llamadafunc'] = Node(LlamadaFunc(self.data, self.funcion), parent = root)
                globals()['argumentos'].parent = globals()['llamadafunc'] 
            globals()['llamada']=1
...
class Termino(Nodo):            
                ...
                if pila[2]=='Definicion':
                    while i == 0: 
                        ...
                        if pila[posi] == 'Definicion':
                            ...
                            i = 1
                            break
                    globals()['termino'] = Node(Termino(self.data, self.tipo, 'return '+contexto), parent = root)
                else:
                    globals()['termino'] = Node(Termino(self.data, self.tipo, 'return '+ pila[4].cad), parent = root)
    ...
    def __repr__(self):
        try:
            aux = ('Termino '+ "\"" + str(self.lv) + "\"" + ' Id: '+str(self.data.cad))
...
class analizador:
    ...
    def reservado(self):
        ...
        if "while" == strid:
            ...
            self.tipo.append(21)
            objlex = terminal(self.tmp, 'return', self.tipo[-1])
            listalexico.append(objlex)
